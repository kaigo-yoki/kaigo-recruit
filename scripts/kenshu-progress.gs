/**
 * 研修進捗管理 - Google Sheets バックエンド v1.0
 * 修了証発行時の自動記録と、進捗確認ページ（kenshu-shinchoku.html）へのデータ提供
 *
 * ▼ セットアップ手順（約5分）
 ...
 * 6. scripts/cert-config.js の KENSHU_PROGRESS_CONFIG.endpoint にURLを貼り付け
 *    （ClaudeにURLを伝えれば、設定からデプロイまで実施します）
 * 7. 「職員名簿」シートに職員を記入（氏名／所属／状態=在籍）
 *
 * ▼ 進捗の確認方法
 * https://kaigo-recruit.vercel.app/kenshu-shinchoku を開いて、
 * 「設定」シート B1 の閲覧キーを入力する
 */

const ROSTER_SHEET = '職員名簿';
...
function setup() {
  ...
  if (!roster) {
    ...
    roster.appendRow(['山田 太郎（記入例・削除可）', '訪問介護', '在籍']);
  ...
  if (!log) {
    ...
    log.getRange(1, 1, 1, LOG_HEADERS.length)
      .setFontWeight('bold').setBackground('#4CAF7D').setFontColor('#ffffff');
    log.setFrozenRows(1);
...
function doPost(e) {
  try {
    ...
    if (!/^\/(rinri-kenshu|(trainings|individual-training|shougai-trainings|kango-trainings)\/[A-Za-z0-9_-]+)$/.test(path)) {
...
function testLog() {
  const e = { postData: { contents: JSON.stringify({
    action: 'log', name: 'テスト 太郎', training: 'テスト研修', path: '/trainings/test', date: '2026-06-12'
  }) } };
